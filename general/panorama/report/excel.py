...
def _severity_sast(findings: list[dict]) -> dict[str, int]:
    counts: dict[str, int] = {}
    for f in findings:
        if (f.get("rule_id") or "").startswith("deps."):
            continue
...
def _severity_sca(vulns: list[dict]) -> dict[str, int]:
    counts: dict[str, int] = {}
    for v in vulns:
        sev = severity_for_vuln(_safe(v.get("vuln_id", "")))
        counts[sev] = counts.get(sev, 0) + 1
    return counts
...
def write_excel(
    ...
    """Write panorama-report.xlsx with sheets FRONTPAGE, SAST, SCA, LICENSES. Returns [(path, size), ...]."""
    wb = Workbook()
    ...
    r += 1
    for label, value in [
        ("SAST findings (code)", len(code_findings)),
        ("SCA vulnerabilities (dependencies)", len(vulns)),
    ...
    # ---- LICENSES: ID (PURL) | Component/Location | Type | License | Extra ----
    ws_lic = wb.create_sheet("LICENSES", 3)
    lic_headers = [
        "PURL", "Component Name", "Version", "Ecosystem", "File", "Line", "Type", "License", "Notes",
    ]
    ...
    for row, c in enumerate(sbom, start=2):
        ...
        ws_lic.cell(row=row, column=5, value=_safe(c.get("file")))
        ws_lic.cell(row=row, column=6, value=c.get("line"))
        ws_lic.cell(row=row, column=7, value=_safe(c.get("type")) or "library")
        ws_lic.cell(row=row, column=8, value=_safe(c.get("license")) or "N/A")
        ws_lic.cell(row=row, column=9, value=_safe(c.get("notes")))
    ...
    ws_lic.column_dimensions["E"].width = 32
    ws_lic.column_dimensions["F"].width = 8
    ws_lic.column_dimensions["G"].width = 10
    ws_lic.column_dimensions["H"].width = 14
    ws_lic.column_dimensions["I"].width = 24
